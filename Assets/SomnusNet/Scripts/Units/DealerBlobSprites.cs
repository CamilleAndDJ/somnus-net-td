...
{
    public static class DealerBlobSprites
    {
        const float PixelsPerUnit = 40f;
        static readonly Vector2 Pivot = new(0.5f, 0.4f);
        static readonly Color Core = new(0.72f, 0.55f, 0.98f);
        static readonly Color Edge = new(0.42f, 0.28f, 0.72f);
        ...
        public static Sprite BodySprite
        {
            get
            {
                ...
                var resource = Resources.Load<Sprite>("DealerBlob");
                if (resource != null)
                {

...
{
    public static class FourOhFourBlobSprites
    {
        const float PixelsPerUnit = 40f;
        static readonly Vector2 Pivot = new(0.5f, 0.4f);
        static readonly Color Core = new(0.62f, 0.84f, 0.98f);
        static readonly Color Edge = new(0.38f, 0.62f, 0.88f);
        ...
        public static Sprite BodySprite
        {
            get
            {
                ...
                var resource = Resources.Load<Sprite>("FourOhFourBlob");
                if (resource != null)
                {

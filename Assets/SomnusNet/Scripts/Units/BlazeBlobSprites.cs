...
{
    public static class BlazeBlobSprites
    {
        const float PixelsPerUnit = 40f;
        static readonly Vector2 Pivot = new(0.5f, 0.4f);
        static readonly Color Core = new(0.98f, 0.42f, 0.28f);
        static readonly Color Edge = new(0.78f, 0.18f, 0.12f);
        ...
        public static Sprite BodySprite
        {
            get
            {
                ...
                var resource = Resources.Load<Sprite>("BlazeBlob");
                if (resource != null)
                {
